<aside class="fixed h-screen bg-[#F0F5F1] top-0 shadow-default z-20 overflow-scroll overflow-x-hidden">
    <section class="flex items-center justify-between gap-4 py-4 pl-3 bg-[#F5FBF7]">
        <div class="avatar">
            <div class="w-10 rounded-full">
                <img src="https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp" />
            </div>
        </div>
        <div>
            <h1>Administrator</h1>
            <p class="text-uppercase">superuser</p>
        </div>
        <div class="dropdown dropdown-hover">
            <div tabindex="0" role="button" class="bg-transparent border-0 btn">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M12 15.0001L7 10.0001H17L12 15.0001Z" fill="#005B4D" />
                </svg>
            </div>
            <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
                <li><a>Item 1</a></li>
                <li><a>Item 2</a></li>
            </ul>
        </div>
    </section>
    <section class="flex flex-col items-center pt-5">
        <a href="<?= site_url('dashboard') ?>" class="btn btn-wide justify-normal <?= $title === 'Dashboard' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full bg-transparent shadow-none text-[#3D4946]' ?> font-normal" style="height: 4rem; min-height: 4rem;">
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M4 21V9L12 3L20 9V21H14V14H10V21H4Z" fill="#3D4946" />
                </svg>
            </div>
            Dashboard
        </a>
        <a href="<?= site_url('dashboard/pengumuman') ?>" class="btn btn-wide justify-normal <?= $title === 'Pengumuman' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full bg-transparent shadow-none text-[#3D4946]' ?> font-normal" style="height: 4rem; min-height: 4rem;">
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M2 10C2 8.33335 2.371 6.80435 3.113 5.41301C3.855 4.02168 4.85067 2.86735 6.1 1.95001L7.275 3.55001C6.275 4.28335 5.47933 5.20835 4.888 6.32501C4.29667 7.44168 4.00067 8.66668 4 10H2ZM20 10C20 8.66668 19.704 7.44168 19.112 6.32501C18.52 5.20835 17.7247 4.28335 16.726 3.55001L17.901 1.95001C19.151 2.86668 20.147 4.02101 20.889 5.41301C21.631 6.80501 22.0013 8.33401 22 10H20ZM4 19V17H6V10C6 8.61668 6.41667 7.38768 7.25 6.31301C8.08333 5.23835 9.16667 4.53401 10.5 4.20001V3.50001C10.5 3.08335 10.646 2.72935 10.938 2.43801C11.23 2.14668 11.584 2.00068 12 2.00001C12.416 1.99935 12.7703 2.14535 13.063 2.43801C13.3557 2.73068 13.5013 3.08468 13.5 3.50001V4.20001C14.8333 4.53335 15.9167 5.23768 16.75 6.31301C17.5833 7.38835 18 8.61735 18 10V17H20V19H4ZM12 22C11.45 22 10.9793 21.8043 10.588 21.413C10.1967 21.0217 10.0007 20.5507 10 20H14C14 20.55 13.8043 21.021 13.413 21.413C13.0217 21.805 12.5507 22.0007 12 22ZM8 17H16V10C16 8.90001 15.6083 7.95835 14.825 7.17501C14.0417 6.39168 13.1 6.00001 12 6.00001C10.9 6.00001 9.95833 6.39168 9.175 7.17501C8.39167 7.95835 8 8.90001 8 10V17Z" fill="#3D4946" />
                </svg>
            </div>
            Pengumuman
        </a>
    </section>
    <section class="border-t-2 border-[#BCC9C4]">
        <details class="w-full dropdown" open>
            <summary class="w-full bg-transparent shadow-none btn" style="height: 4rem; min-height: 4rem;">
                <div class="flex items-center justify-between w-full">
                    <h1 class="text-[#2A5047]">Data Master</h1>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        <path d="M12 15.0001L7 10.0001H17L12 15.0001Z" fill="#005B4D" />
                    </svg>
                </div>
            </summary>
            <ul class="p-2 menu rounded-box">
                <li>
                    <a href="<?= site_url('dashboard/data-master/tahun-akademik') ?>" class="<?= $title === 'Tahun Akademik' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M5 22C4.45 22 3.97933 21.8043 3.588 21.413C3.19667 21.0217 3.00067 20.5507 3 20V6C3 5.45 3.196 4.97933 3.588 4.588C3.98 4.19667 4.45067 4.00067 5 4H6V2H8V4H16V2H18V4H19C19.55 4 20.021 4.196 20.413 4.588C20.805 4.98 21.0007 5.45067 21 6V20C21 20.55 20.8043 21.021 20.413 21.413C20.0217 21.805 19.5507 22.0007 19 22H5ZM5 20H19V10H5V20ZM5 8H19V6H5V8ZM12 14C11.7167 14 11.4793 13.904 11.288 13.712C11.0967 13.52 11.0007 13.2827 11 13C10.9993 12.7173 11.0953 12.48 11.288 12.288C11.4807 12.096 11.718 12 12 12C12.282 12 12.5197 12.096 12.713 12.288C12.9063 12.48 13.002 12.7173 13 13C12.998 13.2827 12.902 13.5203 12.712 13.713C12.522 13.9057 12.2847 14.0013 12 14ZM8 14C7.71667 14 7.47933 13.904 7.288 13.712C7.09667 13.52 7.00067 13.2827 7 13C6.99933 12.7173 7.09533 12.48 7.288 12.288C7.48067 12.096 7.718 12 8 12C8.282 12 8.51967 12.096 8.713 12.288C8.90633 12.48 9.002 12.7173 9 13C8.998 13.2827 8.902 13.5203 8.712 13.713C8.522 13.9057 8.28467 14.0013 8 14ZM16 14C15.7167 14 15.4793 13.904 15.288 13.712C15.0967 13.52 15.0007 13.2827 15 13C14.9993 12.7173 15.0953 12.48 15.288 12.288C15.4807 12.096 15.718 12 16 12C16.282 12 16.5197 12.096 16.713 12.288C16.9063 12.48 17.002 12.7173 17 13C16.998 13.2827 16.902 13.5203 16.712 13.713C16.522 13.9057 16.2847 14.0013 16 14ZM12 18C11.7167 18 11.4793 17.904 11.288 17.712C11.0967 17.52 11.0007 17.2827 11 17C10.9993 16.7173 11.0953 16.48 11.288 16.288C11.4807 16.096 11.718 16 12 16C12.282 16 12.5197 16.096 12.713 16.288C12.9063 16.48 13.002 16.7173 13 17C12.998 17.2827 12.902 17.5203 12.712 17.713C12.522 17.9057 12.2847 18.0013 12 18ZM8 18C7.71667 18 7.47933 17.904 7.288 17.712C7.09667 17.52 7.00067 17.2827 7 17C6.99933 16.7173 7.09533 16.48 7.288 16.288C7.48067 16.096 7.718 16 8 16C8.282 16 8.51967 16.096 8.713 16.288C8.90633 16.48 9.002 16.7173 9 17C8.998 17.2827 8.902 17.5203 8.712 17.713C8.522 17.9057 8.28467 18.0013 8 18ZM16 18C15.7167 18 15.4793 17.904 15.288 17.712C15.0967 17.52 15.0007 17.2827 15 17C14.9993 16.7173 15.0953 16.48 15.288 16.288C15.4807 16.096 15.718 16 16 16C16.282 16 16.5197 16.096 16.713 16.288C16.9063 16.48 17.002 16.7173 17 17C16.998 17.2827 16.902 17.5203 16.712 17.713C16.522 17.9057 16.2847 18.0013 16 18Z" fill="#3D4946" />
                        </svg>
                        Tahun Akademik
                    </a>
                </li>
                <li>
                    <a href="<?= site_url('dashboard/data-master/kelas') ?>" class="<?= $title === 'Kelas' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M4 22V4C4 3.45 4.196 2.97933 4.588 2.588C4.98 2.19667 5.45067 2.00067 6 2H18C18.55 2 19.021 2.196 19.413 2.588C19.805 2.98 20.0007 3.45067 20 4V22H4ZM6 20H18V4H6V20ZM15.5 13.5C15.9167 13.5 16.271 13.3543 16.563 13.063C16.855 12.7717 17.0007 12.4173 17 12C16.9993 11.5827 16.8537 11.2287 16.563 10.938C16.2723 10.6473 15.918 10.5013 15.5 10.5C15.082 10.4987 14.728 10.6447 14.438 10.938C14.148 11.2313 14.002 11.5853 14 12C13.998 12.4147 14.144 12.769 14.438 13.063C14.732 13.357 15.086 13.5027 15.5 13.5Z" fill="#3D4946" />
                        </svg>
                        Kelas
                    </a>
                </li>
                <li>
                    <a href="<?= site_url('dashboard/data-master/siswa') ?>" class="<?= $title === 'Siswa' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M0 18V16.425C0 15.7083 0.366667 15.125 1.1 14.675C1.83333 14.225 2.8 14 4 14C4.21667 14 4.425 14.0043 4.625 14.013C4.825 14.0217 5.01667 14.0423 5.2 14.075C4.96667 14.425 4.79167 14.7917 4.675 15.175C4.55833 15.5583 4.5 15.9583 4.5 16.375V18H0ZM6 18V16.375C6 15.8417 6.146 15.354 6.438 14.912C6.73 14.47 7.14233 14.0827 7.675 13.75C8.20767 13.4173 8.84533 13.1673 9.588 13C10.3307 12.8327 11.1347 12.7493 12 12.75C12.8833 12.75 13.696 12.8333 14.438 13C15.18 13.1667 15.8173 13.4167 16.35 13.75C16.8827 14.0833 17.291 14.471 17.575 14.913C17.859 15.355 18.0007 15.8423 18 16.375V18H6ZM19.5 18V16.375C19.5 15.9417 19.446 15.5333 19.338 15.15C19.23 14.7667 19.0673 14.4083 18.85 14.075C19.0333 14.0417 19.221 14.021 19.413 14.013C19.605 14.005 19.8007 14.0007 20 14C21.2 14 22.1667 14.221 22.9 14.663C23.6333 15.105 24 15.6923 24 16.425V18H19.5ZM8.125 16H15.9C15.7333 15.6667 15.2707 15.375 14.512 15.125C13.7533 14.875 12.916 14.75 12 14.75C11.084 14.75 10.2467 14.875 9.488 15.125C8.72933 15.375 8.275 15.6667 8.125 16ZM4 13C3.45 13 2.97933 12.8043 2.588 12.413C2.19667 12.0217 2.00067 11.5507 2 11C2 10.4333 2.196 9.95833 2.588 9.575C2.98 9.19167 3.45067 9 4 9C4.56667 9 5.04167 9.19167 5.425 9.575C5.80833 9.95833 6 10.4333 6 11C6 11.55 5.80833 12.021 5.425 12.413C5.04167 12.805 4.56667 13.0007 4 13ZM20 13C19.45 13 18.9793 12.8043 18.588 12.413C18.1967 12.0217 18.0007 11.5507 18 11C18 10.4333 18.196 9.95833 18.588 9.575C18.98 9.19167 19.4507 9 20 9C20.5667 9 21.0417 9.19167 21.425 9.575C21.8083 9.95833 22 10.4333 22 11C22 11.55 21.8083 12.021 21.425 12.413C21.0417 12.805 20.5667 13.0007 20 13ZM12 12C11.1667 12 10.4583 11.7083 9.875 11.125C9.29167 10.5417 9 9.83333 9 9C9 8.15 9.29167 7.43767 9.875 6.863C10.4583 6.28833 11.1667 6.00067 12 6C12.85 6 13.5627 6.28767 14.138 6.863C14.7133 7.43833 15.0007 8.15067 15 9C15 9.83333 14.7127 10.5417 14.138 11.125C13.5633 11.7083 12.8507 12 12 12ZM12 10C12.2833 10 12.521 9.904 12.713 9.712C12.905 9.52 13.0007 9.28267 13 9C12.9993 8.71733 12.9033 8.48 12.712 8.288C12.5207 8.096 12.2833 8 12 8C11.7167 8 11.4793 8.096 11.288 8.288C11.0967 8.48 11.0007 8.71733 11 9C10.9993 9.28267 11.0953 9.52033 11.288 9.713C11.4807 9.90567 11.718 10.0013 12 10Z" fill="#3D4946" />
                        </svg>
                        Siswa
                    </a>
                </li>
            </ul>
        </details>
    </section>
    <section class="border-t-2 border-[#BCC9C4]">
        <details class="w-full dropdown" open>
            <summary class="w-full bg-transparent shadow-none btn" style="height: 4rem; min-height: 4rem;">
                <div class="flex items-center justify-between w-full">
                    <h1 class="text-[#2A5047]">Laporan Keuangan</h1>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        <path d="M12 15.0001L7 10.0001H17L12 15.0001Z" fill="#005B4D" />
                    </svg>
                </div>
            </summary>
            <ul class="p-2 menu rounded-box">
                <li>
                    <a href="<?= site_url('dashboard/laporan-keuangan/transaksi-tagihan') ?>" class="<?= $title === 'Transaksi Tagihan' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M6 22C5.45 22 4.97933 21.8043 4.588 21.413C4.19667 21.0217 4.00067 20.5507 4 20V4C4 3.45 4.196 2.97933 4.588 2.588C4.98 2.19667 5.45067 2.00067 6 2H18C18.55 2 19.021 2.196 19.413 2.588C19.805 2.98 20.0007 3.45067 20 4V20C20 20.55 19.8043 21.021 19.413 21.413C19.0217 21.805 18.5507 22.0007 18 22H6ZM6 20H18V4H16V11L13.5 9.5L11 11V4H6V20Z" fill="#3D4946" />
                        </svg>
                        Transaksi Tagihan
                    </a>
                </li>
                <li>
                    <a href="<?= site_url('dashboard/laporan-keuangan/reminder-tagihan') ?>" class="<?= $title === 'Reminder Tagihan' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M17.5856 13.5787L14.5856 12.0787C14.4675 12.0199 14.336 11.9933 14.2043 12.0015C14.0726 12.0097 13.9455 12.0525 13.8356 12.1256L12.4584 13.0444C11.8262 12.6968 11.306 12.1766 10.9584 11.5444L11.8772 10.1672C11.9503 10.0573 11.9931 9.93016 12.0013 9.79848C12.0096 9.6668 11.9829 9.53528 11.9241 9.41719L10.4241 6.41719C10.3619 6.29163 10.2658 6.186 10.1466 6.11226C10.0275 6.03853 9.8901 5.99964 9.74999 6C8.75543 6 7.8016 6.39509 7.09834 7.09835C6.39508 7.80161 5.99999 8.75544 5.99999 9.75C6.00247 11.9373 6.87246 14.0343 8.4191 15.5809C9.96574 17.1275 12.0627 17.9975 14.25 18C14.7424 18 15.2301 17.903 15.6851 17.7145C16.14 17.5261 16.5534 17.2499 16.9016 16.9017C17.2499 16.5534 17.5261 16.14 17.7145 15.6851C17.903 15.2301 18 14.7425 18 14.25C18.0001 14.1107 17.9614 13.9741 17.8882 13.8555C17.815 13.7369 17.7102 13.6411 17.5856 13.5787ZM14.25 16.5C12.4604 16.498 10.7447 15.7862 9.47921 14.5208C8.21377 13.2553 7.50198 11.5396 7.49999 9.75C7.49985 9.2298 7.67996 8.72562 8.00968 8.32326C8.33939 7.9209 8.79834 7.64521 9.30843 7.54313L10.3847 9.69938L9.46874 11.0625C9.4003 11.1652 9.35824 11.2831 9.3463 11.406C9.33437 11.5288 9.35291 11.6526 9.4003 11.7666C9.93691 13.0419 10.9515 14.0565 12.2269 14.5931C12.3411 14.6426 12.4659 14.6629 12.59 14.6521C12.7141 14.6413 12.8335 14.5998 12.9375 14.5312L14.3072 13.6181L16.4634 14.6944C16.3605 15.205 16.0837 15.6642 15.68 15.9935C15.2763 16.3228 14.7709 16.5018 14.25 16.5ZM12 2.25C10.3167 2.24963 8.66195 2.68508 7.19685 3.51396C5.73176 4.34285 4.50622 5.53692 3.63953 6.97997C2.77283 8.42301 2.29451 10.0659 2.25111 11.7486C2.20771 13.4314 2.60071 15.0967 3.39187 16.5825L2.3278 19.7747C2.23967 20.039 2.22687 20.3226 2.29086 20.5938C2.35486 20.8649 2.4931 21.1129 2.6901 21.3099C2.88711 21.5069 3.13508 21.6451 3.40624 21.7091C3.67739 21.7731 3.96101 21.7603 4.2253 21.6722L7.41749 20.6081C8.72509 21.3036 10.1739 21.6921 11.6541 21.744C13.1342 21.7959 14.6067 21.51 15.9599 20.9079C17.313 20.3057 18.5112 19.4033 19.4634 18.269C20.4157 17.1346 21.097 15.7983 21.4557 14.3613C21.8144 12.9244 21.841 11.4246 21.5336 9.97578C21.2261 8.52701 20.5926 7.16732 19.6811 5.99992C18.7697 4.83253 17.6043 3.88811 16.2734 3.23836C14.9425 2.58861 13.481 2.25061 12 2.25ZM12 20.25C10.5497 20.251 9.12477 19.8691 7.86937 19.1428C7.77745 19.0895 7.67526 19.0563 7.56955 19.0455C7.46385 19.0347 7.35706 19.0464 7.25624 19.08L3.74999 20.25L4.91905 16.7438C4.95275 16.643 4.96466 16.5362 4.954 16.4305C4.94333 16.3248 4.91033 16.2226 4.85718 16.1306C3.94778 14.5584 3.58266 12.7299 3.81846 10.929C4.05426 9.12801 4.8778 7.4552 6.16132 6.17006C7.44485 4.88491 9.11661 4.05925 10.9173 3.82118C12.7179 3.5831 14.5468 3.94591 16.1202 4.85333C17.6937 5.76074 18.9236 7.16204 19.6194 8.83983C20.3151 10.5176 20.4377 12.3781 19.9682 14.1327C19.4987 15.8873 18.4632 17.4379 17.0225 18.5439C15.5818 19.65 13.8163 20.2497 12 20.25Z" fill="#3D4946" />
                        </svg>
                        Reminder Tagihan
                    </a>
                </li>
                <li>
                    <a href="<?= site_url('dashboard/laporan-keuangan/atur-jenis-tagihan') ?>" class="<?= $title === 'Atur Jenis Tagihan' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M9 18H20V15.325H9V18ZM4 8.675H7V6H4V8.675ZM4 13.35H7V10.675H4V13.35ZM4 18H7V15.325H4V18ZM9 13.35H20V10.675H9V13.35ZM9 8.675H20V6H9V8.675ZM4 20C3.45 20 2.97933 19.8043 2.588 19.413C2.19667 19.0217 2.00067 18.5507 2 18V6C2 5.45 2.196 4.97933 2.588 4.588C2.98 4.19667 3.45067 4.00067 4 4H20C20.55 4 21.021 4.196 21.413 4.588C21.805 4.98 22.0007 5.45067 22 6V18C22 18.55 21.8043 19.021 21.413 19.413C21.0217 19.805 20.5507 20.0007 20 20H4Z" fill="#3D4946" />
                        </svg>
                        Atur Jenis Tagihan
                    </a>
                </li>
                <li>
                    <a class="rounded-full text-[#3D4946] font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M4 20C3.45 20 2.97917 19.8042 2.5875 19.4125C2.19583 19.0208 2 18.55 2 18V6C2 5.45 2.19583 4.97917 2.5875 4.5875C2.97917 4.19583 3.45 4 4 4H10L12 6H20C20.55 6 21.0208 6.19583 21.4125 6.5875C21.8042 6.97917 22 7.45 22 8V18C22 18.55 21.8042 19.0208 21.4125 19.4125C21.0208 19.8042 20.55 20 20 20H4ZM4 18H20V8H11.175L9.175 6H4V18Z" fill="#3D4946" />
                        </svg>
                        Rekapitulasi
                    </a>
                </li>
            </ul>
        </details>
    </section>
    <section class="border-t-2 border-[#BCC9C4]">
        <details class="w-full dropdown" open>
            <summary class="w-full bg-transparent shadow-none btn" style="height: 4rem; min-height: 4rem;">
                <div class="flex items-center justify-between w-full">
                    <h1 class="text-[#2A5047]">Pengaturan Sistem</h1>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        <path d="M12 15.0001L7 10.0001H17L12 15.0001Z" fill="#005B4D" />
                    </svg>
                </div>
            </summary>
            <ul class="p-2 menu rounded-box">
                <li>
                    <a href="<?= site_url('dashboard/pengaturan-sistem/informasi-sekolah') ?>" class="<?= $title === 'Informasi Sekolah' ? 'rounded-full bg-[#C4EDE1] text-[#2A5047]' : 'rounded-full text-[#3D4946]' ?> font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                            <path d="M10 5C10.2833 5 10.521 4.904 10.713 4.712C10.905 4.52 11.0007 4.28267 11 4C10.9993 3.71733 10.9033 3.48 10.712 3.288C10.5207 3.096 10.2833 3 10 3C9.71667 3 9.47933 3.096 9.288 3.288C9.09667 3.48 9.00067 3.71733 9 4C8.99933 4.28267 9.09533 4.52033 9.288 4.713C9.48067 4.90567 9.718 5.00133 10 5ZM9 13H11V7H9V13ZM0 20V2C0 1.45 0.196 0.979333 0.588 0.588C0.98 0.196667 1.45067 0.000666667 2 0H18C18.55 0 19.021 0.196 19.413 0.588C19.805 0.98 20.0007 1.45067 20 2V14C20 14.55 19.8043 15.021 19.413 15.413C19.0217 15.805 18.5507 16.0007 18 16H4L0 20ZM3.15 14H18V2H2V15.125L3.15 14Z" fill="#3D4946" />
                        </svg>
                        Informasi Sekolah
                    </a>
                </li>
                <li>
                    <a class="rounded-full text-[#3D4946] font-normal content-center" style="height: 4rem; min-height: 4rem;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M17 17C17.4167 17 17.771 16.8543 18.063 16.563C18.355 16.2717 18.5007 15.9173 18.5 15.5C18.4993 15.0827 18.3537 14.7287 18.063 14.438C17.7723 14.1473 17.418 14.0013 17 14C16.582 13.9987 16.228 14.1447 15.938 14.438C15.648 14.7313 15.502 15.0853 15.5 15.5C15.498 15.9147 15.644 16.269 15.938 16.563C16.232 16.857 16.586 17.0027 17 17ZM17 20C17.5167 20 17.9917 19.879 18.425 19.637C18.8583 19.395 19.2083 19.0743 19.475 18.675C19.1083 18.4583 18.7167 18.2917 18.3 18.175C17.8833 18.0583 17.45 18 17 18C16.55 18 16.1167 18.0583 15.7 18.175C15.2833 18.2917 14.8917 18.4583 14.525 18.675C14.7917 19.075 15.1417 19.396 15.575 19.638C16.0083 19.88 16.4833 20.0007 17 20ZM12 22C9.68333 21.4167 7.77067 20.0873 6.262 18.012C4.75333 15.9367 3.99933 13.6327 4 11.1V5L12 2L20 5V10.675C19.6833 10.5417 19.3583 10.4207 19.025 10.312C18.6917 10.2033 18.35 10.1247 18 10.076V6.4L12 4.15L6 6.4V11.1C6 11.8833 6.10433 12.6667 6.313 13.45C6.52167 14.2333 6.81333 14.9793 7.188 15.688C7.56267 16.3967 8.01667 17.0507 8.55 17.65C9.08333 18.2493 9.675 18.7493 10.325 19.15C10.5083 19.6833 10.75 20.1917 11.05 20.675C11.35 21.1583 11.6917 21.5917 12.075 21.975C12.0583 21.975 12.046 21.9793 12.038 21.988C12.03 21.9967 12.0173 22.0007 12 22ZM17 22C15.6167 22 14.4377 21.5123 13.463 20.537C12.4883 19.5617 12.0007 18.3827 12 17C11.9993 15.6173 12.487 14.4383 13.463 13.463C14.439 12.4877 15.618 12 17 12C18.382 12 19.5613 12.4877 20.538 13.463C21.5147 14.4383 22.002 15.6173 22 17C21.998 18.3827 21.5103 19.562 20.537 20.538C19.5637 21.514 18.3847 22.0013 17 22Z" fill="#3D4946" />
                        </svg>
                        Manajemen User
                    </a>
                </li>
            </ul>
        </details>
    </section>
</aside>