...
<header class="flex items-center justify-between px-10">
    <form action="<?= site_url('dashboard/data-master/tahun-akademik') ?>" method="get" class="flex items-center justify-between">
        <?= csrf_field() ?>
        <div class="flex gap-4">
            <label class="relative flex items-center gap-2 px-1 pl-5 rounded-full input border-[#6D7A75] h-14"><span class="absolute px-4 text-sm bg-white bottom-12">Cari</span>
                <input type="text" name="tahun_akademik" class="grow" placeholder="Search" value="<?= $tahun_akademik ?? '' ?>" />
...
<dialog id="my_modal_1" class="modal">
    <div class="modal-box">
        <h3 class="text-lg font-bold">Tambah Tahun Akademik</h3>
        <form method="dialog">
            <button class="absolute btn btn-sm btn-circle btn-ghost right-2 top-2">✕</button>
        </form>
        <form action="<?= site_url('dashboard/data-master/tahun-akademik') ?>" method="post" class="flex flex-col gap-5 py-10">
            <?= csrf_field() ?>
            <label class="relative flex items-center gap-2 px-5 rounded-full input border-[#6D7A75] bg-transparent h-14"><span class="absolute px-4 text-sm bg-[#F0F5F1] bottom-12">Tahun Sekarang</span>
                <input type="date" class="grow" name="tahun_sekarang" value="<?= old('tahun_sekarang') ?>" />
            </label>
            <label class="relative flex items-center gap-2 px-5 rounded-full input border-[#6D7A75] bg-transparent h-14"><span class="absolute px-4 text-sm bg-[#F0F5F1] bottom-12">Tahun Selanjutnya</span>
                <input type="date" class="grow" name="tahun_selanjutnya" value="<?= old('tahun_selanjutnya') ?>" />
            </label>
            <label class="relative flex items-center gap-2 px-5 rounded-full input border-[#6D7A75] bg-transparent h-14"><span class="absolute px-4 text-sm bg-[#F0F5F1] bottom-12">Status Periode</span>
                <select name="status_periode" class="w-full bg-transparent grow">
...
<section class="grid grid-flow-row-dense grid-cols-3 grid-rows-1 gap-10 px-10 mt-10">
            ...
            <table class="table">
                ...
                <tbody class="text-center text-[#005B4D] text-wrap">
                    ...
                    <?php foreach ($tahunAkademik as $tahun) : ?>
                        <tr>
                            <th><?= $no++ ?></th>
                            <td><span><?= $tahun['tahun_sekarang'] ?></span>/<span><?= $tahun['tahun_selanjutnya'] ?></span></td>
                            <td><?= $tahun['status_periode'] ?></td>
                            <td>
                                <div class="flex gap-4">
                                    <a href="#" class="edit-button" data-id="<?= $tahun['id']; ?>" data-tahun-sekarang="<?= $pengum['tahun_sekarang']; ?>" data-tahun-selanjutnya="<?= $tahun['tahun_selanjutnya']; ?>">
                                    ...
                                    <form action="<?= site_url('dashboard/data-master/tahun-akademik/' . $pengum['id']) ?>" method="post">
                                        <?= csrf_field() ?>
    ...
    <article class="flex flex-col gap-10 bg-[#F0F5F1] p-5 rounded-2xl">
        <header>
            <h1 class="text-2xl font-bold text-[#005B4D]">Atur Tahun Akademik</h1>
        </header>
        <form action="<?= site_url('dashboard/data-master/tahun-akademik') ?>" method="post" class="flex flex-col gap-10">
            <?= csrf_field() ?>
            <label class="relative flex items-center gap-2 px-5 rounded-full input border-[#6D7A75] bg-transparent h-14"><span class="absolute px-4 text-sm bg-[#F0F5F1] bottom-12">Tahun Sekarang</span>
                <input type="date" class="grow" name="tahun_sekarang" value="<?= old('tahun_sekarang') ?>" />
            </label>
